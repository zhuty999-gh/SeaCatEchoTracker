...
- **时空畸体的施加延迟**。球飞行途中才逐个施加，插件按施放时刻计算。第一个被经过的盟友几乎立即获得 Echo，所以对「最早到期」的影响小于 1 秒，且方向偏保守（真实到期比显示的更晚）。
- **Echo 总数是估算值,默认只在团队副本内显示**。时空畸体一次给最多 5 个盟友上 Echo,插件不知道球实际经过了谁。0.4.1 曾整体移除计数,0.5.0 以「团队副本内按固定 5 计算」的形式恢复——20 人团里球几乎必然清满 5 个,这个近似站得住。
